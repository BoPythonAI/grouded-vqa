...
LOG="${VQA_LOG_ROOT}/e17_adapter_scope_10k_$(date +%Y%m%d_%H%M%S).log"

exec > >(tee -a "${LOG}") 2>&1
echo "E17 fixed-10k adapter-scope sequence started at $(date --iso-8601=seconds)"

for SPEC in \
  "E17b_instructblip_qformer_lora_r8_train10k_standard:qformer" \
  "E17c_instructblip_dual_lora_r8_train10k_standard:dual"
do
  RUN_NAME="${SPEC%%:*}"
  SCOPE="${SPEC##*:}"
...
echo "E17 fixed-10k adapter-scope sequence completed at $(date --iso-8601=seconds)"
